...
class loDCR(parameter):
    ...
    def __init__(self, value=0., unit='', parent='', cpacsPath=''):
        super(loDCR, self).__init__(value=value, unit=unit, doc=self.__doc__, status='init', parent=parent,
                                    cpacsPath=cpacsPath)

    def calc(self):
        '''
        Calculates the lift over drag ratio for the cruise condition from cD and cL for cruise
        '''
        cDCR = self.parent.cDCR.getValue()
        cLCR = self.parent.cLCR.getValue()

        return self.setValueCalc(cLCR / cDCR)
