...
class twFAR25121b(parameter):
    '''
    The thrust to weight ratio for the second segment climb 
    ...
    def __init__(self, value=0., unit='kg/m2', parent='', cpacsPath=''):
        super(twFAR25121b, self).__init__(value=value, unit=unit, doc=self.__doc__, status='init', parent=parent,
                                          cpacsPath=cpacsPath)

    def calc(self):
        '''
        Calculates the Thrust to Weight ratio for the second segment climb 
        ...
        if nEngine == 2.:
            climbGradient = 0.024 # needs to be positive
        elif nEngine == 3.:
            climbGradient = 0.027
        elif nEngine == 4.:
            climbGradient = 0.03
        ...
        #as FAR 25.121b asks for v at v2, CLTO needs to be corrected
        cL = cLTO / 1.2 ** 2.
        cD = cD0 + dCDTO + dCDOEI + cL ** 2. / (aspectRatio * pi * oswald)
