...
class sTOFL(parameter):
    '''
    The take-off field length for airport conditions, i.e. influenced by temperature and altitude 
    
    :Unit: [m]
    '''

    def __init__(self, value=0., unit='m', parent='', cpacsPath=''):
        super(sTOFL, self).__init__(value=value, unit=unit, doc=self.__doc__, status='init', parent=parent,
                                    cpacsPath=cpacsPath)
        self.x1 = 0.
    ...
    def calcRoskam(self):
        '''
        Calculates the takeoff field length via the wingloading and the thrust to weight ratio

        :Source: Airplane Design Part I, J. Roskam, DARCorporation, 2005, Fourth Edition, p.98, eq. 3.7
        ...
        cLTO = self.parent.cLTO.getValue()
        twTO = self.parent.twTO.getValue()

        return self.setValueCalc(corr * wsTO / (cLTO * twTO))
