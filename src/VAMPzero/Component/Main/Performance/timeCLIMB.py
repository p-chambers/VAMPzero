...
class timeCLIMB(parameter):
    ...
    def __init__(self, value=0., unit='s', parent='', cpacsPath=''):
        super(timeCLIMB, self).__init__(value=value, unit=unit, doc=self.__doc__, status='init', parent=parent,
                                        cpacsPath=cpacsPath)

    def calc(self):
        '''
        Calculates the time for the climb segment
        ...
        gamma = self.parent.gammaCLIMB.getValue()
