...
class thrustTO(parameter):
    '''
    Takeoff thrust for one engine. Conditions for airport level
    
    :Unit: [N]
	'''

    def __init__(self, value=0., unit='N', parent='', cpacsPath=''):
        super(thrustTO, self).__init__(value=value, unit=unit, doc=self.__doc__, status='init', parent=parent,
                                       cpacsPath=cpacsPath)
    ...
    def calc(self):
        ...
        if self.parent.thrustTOISA.getStatus() == 'fix':
            self.calc = self.calcThrustISA
        else: 
    ...
    def calcThrustISA(self):
        '''
        Calculates the takeoff thrust from the takeoff thrust at ISA conditions
        ...
        thrustCorr = thrustTOISA * ( pAP / 101325.000 * (1 - 2 * (TAP - 288.15) / 288.15))
    ...
    def calctwTO(self):
        ...
        This calculation does take the airport level and deltaT into account.

        :Source: Airplane Design Part II, J. Roskam, DARCorporation, 2005, Fourth Edition, p. 98
        
        '''
        self.setDeviation(0.054) # needs to be set by the calc method when called for later use (deviation depends on the calc method)
        
        mTOM = self.parent.aircraft.mTOM.getValue()
        TWto = self.parent.aircraft.twTO.getValue()
        n = self.parent.nEngine.getValue()

        thrust00 = TWto * mTOM * 9.81 / n

        return self.setValueCalc(thrust00)
