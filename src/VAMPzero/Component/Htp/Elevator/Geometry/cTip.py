...
class cTip(parameter):
    '''
    Calculates the tip chord of the elevator (note: only one elevator is considered).
    ...
    def __init__(self, value=0., unit='m', parent='', cpacsPath=''):
        super(cTip, self).__init__(value=value, unit=unit, doc=self.__doc__, status='init', parent=parent,
                                   cpacsPath=cpacsPath)
    ...
    def calc(self):
        ...
        cRootElevator = self.parent.cRoot.getValue()

        cTipElevator = taperRatioHtp * cRootElevator

        return self.setValueCalc(cTipElevator)
